...
RTC::ReturnCode_t PresentationMouseDecoder::onExecute(RTC::UniqueId ec_id)
{
  ...
  if(m_MouseEventIn.isNew()){
    ...
    if(m_SlideChangeClick==2){ //Double click detection case
      if(m_MouseEvent.data[0]==CV_EVENT_LBUTTONDBLCLK){
        m_SlideRelativeCommand.data = 1;
        change = true;
      }else if(m_MouseEvent.data[0]==CV_EVENT_RBUTTONDBLCLK){
        m_SlideRelativeCommand.data = -1;
        change = true;
      }
      ...
      if((m_MouseEvent.data[0]==CV_EVENT_LBUTTONDOWN)||((m_MouseEvent.data[0]==CV_EVENT_RBUTTONDOWN))){ //Button down
        if(click_type != 0){ //if other key already pressed - both left and right keys are being pressed
          ...
          if(m_MouseEvent.data[0]==CV_EVENT_LBUTTONDOWN && ((m_MouseEvent.data[3] == CV_EVENT_FLAG_LBUTTON)||(m_MouseEvent.data[3] == 0))){click_type = 1;} //set corresponding click type - left click
          else if(m_MouseEvent.data[3] == CV_EVENT_FLAG_RBUTTON || m_MouseEvent.data[3] == 0){click_type = -1;} // - right click
      ...
      }else if(((click_type==1)&&((m_MouseEvent.data[0]==CV_EVENT_LBUTTONUP)||((m_MouseEvent.data[3]&CV_EVENT_FLAG_LBUTTON)==0)))||((click_type==-1)&&((m_MouseEvent.data[0]==CV_EVENT_RBUTTONUP)||((m_MouseEvent.data[3]&CV_EVENT_FLAG_RBUTTON)==0)))){ //if button released
    ...
    if(m_MouseEvent.data[3] != CV_EVENT_FLAG_LBUTTON){ //button released - clear old position
      x_old = -1;
      y_old = -1;
    }else{
